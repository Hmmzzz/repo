...
if len(version_rows) != 1 or version_rows[0].get("text") != "0.2.8":
    raise SystemExit("Smoke test failed: MarkTheme detail version must be 0.2.8.")
...
if "MarkTheme 0.2.8" not in depiction_text or "MarkTheme 0.2.8" not in web_text:
...
for name, content in (("Sileo", depiction_text), ("web", web_text)):
    for phrase in ("Anemone 风格图标 overlay", "装饰画布", "归一化", "精确适配", "图标服务"):
        if phrase not in content:
            raise SystemExit(f"Smoke test failed: MarkTheme {name} depiction is missing {phrase}.")
    for stale_phrase in (
        "系统 build 精确启用",
...
expected_variants = {
    ...
    for version in ("0.1.0", "0.1.1", "0.1.2", "0.1.3", "0.1.4", "0.1.5", "0.1.6", "0.1.7", "0.1.8", "0.1.9", "0.2.0", "0.2.1", "0.2.2", "0.2.3", "0.2.5", "0.2.6", "0.2.7", "0.2.8")
