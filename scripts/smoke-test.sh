...
if len(version_rows) != 1 or version_rows[0].get("text") != "0.2.1":
    raise SystemExit("Smoke test failed: MarkTheme detail version must be 0.2.1.")
...
if "MarkTheme 0.2.1" not in depiction_text or "MarkTheme 0.2.1" not in web_text:
...
for name, content in (("Sileo", depiction_text), ("web", web_text)):
    for phrase in ("iOS 17.0", "display scale", "文件夹", "MarkTheme-Diagnostics.txt"):
        if phrase not in content:
            raise SystemExit(f"Smoke test failed: MarkTheme {name} depiction is missing {phrase}.")
    for stale_phrase in (
        "系统 build 精确启用",
...
expected_variants = {
    ...
    for version in ("0.1.0", "0.1.1", "0.1.2", "0.1.3", "0.1.4", "0.1.5", "0.1.6", "0.1.7", "0.1.8", "0.1.9", "0.2.0", "0.2.1")
